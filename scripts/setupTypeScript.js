...
function replaceInFile(fileName, replacements) {
  if (fs.existsSync(fileName)) {
    ...
    if (hadUpdates) {
      ...
      console.log(
        `${fileName} had already been updated.`
      );
    }
  } else {
    warn(
      `Wanted to update ${fileName} but the file did not exist.`
    );
  }
}
...
function changeJsExtensionToTs(dir) {
  ...
  for (let i = 0; i < elements.length; i++) {
    if (elements[i].isDirectory()) {
      changeJsExtensionToTs(
        path.join(dir, elements[i].name)
      );
    } else if (
      elements[i].name.match(/^[^_]((?!json).)*js$/)
    ) {
      fs.renameSync(
        path.join(dir, elements[i].name),
        path
          .join(dir, elements[i].name)
          .replace('.js', '.ts')
      );
    }
  }
...
function updateSingleSvelteFile({
  view,
  vars,
  contextModule
}) {
  replaceInFile(
    path.join(projectRoot, 'src', `${view}.svelte`),
    [
      [
        /(?:<script)(( .*?)*?)>/gm,
        (m, attrs) =>
          `<script${attrs}${
            !attrs.includes('lang="ts"')
              ? ' lang="ts"'
              : ''
          }>`
      ],
...
console.log(
  `Adding TypeScript with ${
    isRollup ? 'Rollup' : 'webpack'
  }...`
);
